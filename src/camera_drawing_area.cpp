	...
	drawMode(CENTER_FIT)
{
	...
	set_draw_func(sigc::mem_fun(*this,& CvDrawer::on_draw));
}
...
void CvDrawer::on_draw(const::Cairo::RefPtr<Cairo::Context> &cr, int width, int height){
	...
	cv::Mat mat1;
	cv::cvtColor (mat, mat1, cv::COLOR_BGR2RGB);
	...
	Gdk::Cairo::set_source_pixbuf (cr, Gdk::Pixbuf::create_from_data(mat1.data, Gdk::Colorspace::RGB, false, 8, mat1.cols, mat1.rows, mat1.step));
